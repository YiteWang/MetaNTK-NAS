...
def get_cell_based_tiny_net(config):
    ...
    # Now MetaNTK only supports the following two types:
    if config.ntk_type == 'MetaNTK':
        assert ((super_type == 'basic' and config.name in group_names) or (super_type == 'nasnet-super'))
    ...
    if super_type == 'basic' and config.name in group_names:
        ...
        return nas_super_nets[config.name](config.C, config.N, config.steps, config.multiplier, \
                                        config.stem_multiplier, config.feature_scale_rate, config.num_classes, config.space, config.affine, config.track_running_stats, config.depth, config.use_stem)
